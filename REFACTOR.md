...
## R7.11. Два источника истины: зеркала статов в токенах. P2, M (срез 1 сделан).
...
- **Срез 2:** `syncSheetToTokens` → сборщик без записи; `visibleToken` подставляет resolved-статы (с redaction); `token:update` игнорирует производные поля у персонажа, HP-патчи идут в ресурсы; миграция в `roomNormalize` (выкинуть зеркала у токенов с контролёром); заморозка статов при отвязке персонажа (`player:setCharacter`).
- **Срез 3:** клиент — у токена персонажа статы редактируются только в листе (меню токена показывает их read-only), скрыть `initiativeBonus` у токенов игрока; доки.
**Решения владельца:** имя из листа; при отвязке копируем статы в токен; инициатива автоматом из Ловкости; `owner` у призывов оставляем до полноценной системы призывов.
