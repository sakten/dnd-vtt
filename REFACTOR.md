...
## R7.9. CSS: плоский неймспейс, хардкод-палитра, разрозненные z-index, связь с e2e. P3, S/M (частично закрыто).
Сделано: палитра сведена в `base.css` — 150 хардкод-вхождений заменены на `--*`-токены (фоны, текст, warn/danger/ok/magic/indigo/gold; в правилах hex не хардкодим); z-index — документированная шкала `--z-*` (16 слоёв, 29 значений), значения сохранены как были (порядок слоёв не меняли).
Осталось: `data-testid` на e2e-критичных узлах и миграция селекторов (133 DOM-селектора в `scripts/e2e|smoke`; чаще всего `.modal` 26×, `.chat-msg.roll` 9×, `.initiative-bar` 6×, `.token-panel-item img` 6×, `.sheet-modal`, `.fog-panel`, `.map-item`, `.admin-room*`); единичные тёмные оттенки (`#171b22` и т.п.) не токенизированы осознанно.
