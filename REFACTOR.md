...
- [ ] **R6.7. Типовые швы: `Room` vs `RoomState`, мутационная гидратация, слабая валидация.** P2, M/L. **(срезы 1–4 сделаны: нормализаторы + `isRecord`, `noUncheckedIndexedAccess` включён везде)**
  ...
  **Осталось:** `toPersistedRoom` — снапшот вместо живых ссылок; ревизия `Room`/`RoomState`/`PersistedRoom` (наружу только `RoomState`); декодеры payload сверх `isRecord`.
...
### Шаг 3. R6.7 — типовые швы (M/L, server/shared). Срезы 1–4 ✅ (нормализаторы + `isRecord`, `noUncheckedIndexedAccess` в base)
...
**Осталось:** `toPersistedRoom`-снапшот, ревизия `Room`/`RoomState`/`PersistedRoom`, декодеры payload сверх `isRecord`.
...
**Старт:** R8.2, R6.1–R6.5, R6.6, R6.8, R7.1, R7.2, R7.4, R6.7 (срезы 1–4) — сделано (R6.9 отложен). Следующий — добить R6.7 (снапшот `toPersistedRoom` + ревизия форм комнаты) либо шаг 4 (R8.5).
