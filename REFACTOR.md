...
- [ ] **R6.7. Типовые швы: `Room` vs `RoomState`, мутационная гидратация, слабая валидация.** P2, M/L. **(срезы 1–3 сделаны: нормализаторы + `isRecord`, флаг в shared и server)**
  ...
  **Осталось:** `noUncheckedIndexedAccess` для client (~97 ошибок), затем перенос флага в `tsconfig.base.json`; `toPersistedRoom` — снапшот вместо живых ссылок; ревизия `Room`/`RoomState`/`PersistedRoom` (наружу только `RoomState`); декодеры payload сверх `isRecord`.
...
### Шаг 3. R6.7 — типовые швы (M/L, server/shared). Срезы 1–3 ✅ (нормализаторы + `isRecord`, флаг в shared и server)
...
**Осталось:** `noUncheckedIndexedAccess` для client (затем флаг в base), `toPersistedRoom`-снапшот, ревизия `Room`/`RoomState`/`PersistedRoom`, декодеры payload сверх `isRecord`.
...
**Старт:** R8.2, R6.1–R6.5, R6.6, R6.8, R7.1, R7.2, R7.4, R6.7 (срезы 1–3) — сделано (R6.9 отложен). Следующий — срез 4 R6.7: `noUncheckedIndexedAccess` в client.
