...
> **Очередь:** R7.7, R7.10, R9.1–R9.4, R10, R11, R12, R14, R15 (карточки ниже — единственный источник деталей).
...
## R15. Banishment: возврат может встать в стену. P2, S (из сессии 24.09.2026).
`placeReturningToken`/`nearestFreeSpot` (`server/src/room/effects.ts`) при возврате из изгнания проверяют только занятость клеток токенами: если исходную клетку успели перекрыть стеной/закрытой дверью, пока цель была изгнана, токен возвращается прямо в стену; кольцевой поиск «ближайшего свободного» места тоже не смотрит на стены — может выбрать позицию за стеной или с пересечением стены.
**Что сделать:** при проверке позиций учитывать `map.walls` (`rectCrossesWalls(подошва, walls, 'move')`), при заблокированной исходной клетке расширять кольца (сейчас до 8), при отсутствии вариантов — оставить в исходной точке и предупредить DM системным сообщением.
**Проверка:** `rooms.test.ts`: стена на исходной клетке → возврат в ближайшую свободную без стены; возврат в стену невозможен (1×1 и 2×2).
**Не трогать:** правила `crossesWalls` (закрытая дверь блокирует проход, открытая — нет) и текущую семантику «изгнанный клеток не занимает».
