...
**R8.8 — партия 5 (монах, ядро) ✅:** Шквал ударов — счётчик `TurnState.flurryAttacks` (2 удара, с 10 ур. — 3; учтён в `canAttack/consumeAttack` и клиенте); Терпеливая оборона — Отход + Уклонение, Шаг ветра — Отход + Рывок (новые `AutomationUtility` kinds); пассивки: Защита без доспехов (AC `10+dex+wis`), Движение без доспехов (+10/+15 фт), Extra Attack/Просветлённые удары — native; Ошеломляющий удар — rider с CON-спасом и `stunned` (при успехе — скорость ×1/2), активируется кнопкой-меткой; Отражение атак — реакция `reduceDamage` с бонусами Ловкости и уровня монаха, при полном снижении — окно перенаправления (DEX-спас, 2 кости боевых искусств + Ловкость, дистанция 5/60 фт); `martialArtsDie` в shared. Иконки 6 кнопок монаха. Тесты: shared 13 (+4), server 74 (+3). **Дальше:** подклассы монаха XPHB-четвёрки (openHand/shadow/mercy/fourElements).
