...
6. **Лимит «1 минута» (решение владельца):** длительность ровно 1 минута у концентрационных эффектов **лимитируется 10 раундами**; то, что больше минуты (10 минут/час/сутки), — не лимитируется. Сейчас счётчика нет: `untilSave`/`concentration` живут до снятия концентрации и повторного спаса. Нужна совместимость лимита раундов с повторным спасом (`maxRounds` у длительности или счётчик каста). Затронуты XPHB-смайты Wrathful/Blinding/Shining, каталожные Bless/Bane/Hold Person/Hold Monster и др.; Banishment уже ограничен `rounds: 10`.
